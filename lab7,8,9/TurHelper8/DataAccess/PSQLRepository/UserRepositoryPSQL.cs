    ...
    public class UserRepositoryPSQL : UserRepositoryInterface
    {
        ...
        public Users GetUserByLogin(string Login)
        {
            try
            {
                ...
                {
                    log.Loging("Info: Get User by login - " + Login);
                    var res = dbcon.Users.Where(u => u.Login == Login);
        ...
        public void UpdateUser(Users u)
        {
            try
            {
                using (PSQLContext dbcon = new PSQLContext())
                {
                    var User = dbcon.Users.Find(u.Id);
                    User.Permission = u.Permission;
                    dbcon.Entry(User).State = EntityState.Modified;
                    dbcon.SaveChanges();
                }
